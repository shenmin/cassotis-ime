...
- 持续改进语料训练驱动的本地语言模型，并探索可选的 LLM 辅助输入能力。
...
统计模型经过量化后写入本地词库数据库，神经重排器则导出为确定性的原生 Pascal 参数。用户输入长句时，引擎会在生成当次候选的过程中直接执行这个紧凑的前馈网络，对完整候选进行保守重排。这是本地推理而非训练过程：不会启动 PyTorch/ONNX 运行环境或外部模型服务，不需要联网或 GPU，短词精确匹配排序也不会调用它。
